...
PNG_DIRS:=no-images no-lines page-seg-input render_txt render_pad composited boxes montage_2x2
...
montage2x2_completed.txt: montage_2x2.sh render_pad_completed.txt boxes_completed.txt pdf2png_completed.txt
...
.PHONY: clean
clean:
	rm --force -- $(VIDEO_OUT) $(VIDEO_OUT_NO_AUDIO) $(COMPLETED) jpg/*.jpg tesseract/*.pdf tesseract/*.txt tesseract/*.tsv $(PNG_GLOB)
